...
class MkDocsGallery {
  constructor(containerId, options = {}) {
    ...
    // 自动适配仓库前缀
    this.repoPrefix = window.location.pathname.startsWith('/nm-wiki/') ? '/nm-wiki/' : '/';
  ...
  async loadImages() {
    try {
      ...
      images.forEach(img => {
        ...
        // 自动适配本地和线上路径
        imgElement.src = this.repoPrefix + img;
  ...
  async getImageList() {
    try {
      // 自动适配本地和线上 file-list.json 路径
      const fileListUrl = this.repoPrefix + 'file-list.json';
